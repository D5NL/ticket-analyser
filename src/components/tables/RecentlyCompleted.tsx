...
export const RecentlyCompleted: React.FC<Props> = ({ tickets }) => {
  // Filter en sorteer afgeronde tickets
  const recentlyCompleted = tickets
    .filter(ticket => ticket.status === 'Afgerond')
    .sort((a, b) => {
      const dateA = new Date(a.afgerondeOp || '');
      const dateB = new Date(b.afgerondeOp || '');
        ...
        <table className="min-w-full divide-y divide-gray-200">
          ...
          <tbody className="divide-y divide-gray-200">
            {recentlyCompleted.map((ticket, index) => (
              <tr key={ticket._id || ticket.meldingsnummer || `recent-completed-${index}`}>
